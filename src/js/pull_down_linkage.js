...
function change(v){ 
	var str="0"; 
	for(let i=0;i<v;i++){ str+=("_"+(document.getElementById(s[i]).selectedIndex-1));}; 
	...
	if(v && document.getElementById(s[v-1]).selectedIndex>0 || !v) { 
		if(dsy.Exists(str)){ 
		let ar = dsy.Items[str]; 
		console.log(ar)
		for(let i=0;i<ar.length;i++) ss.options[ss.length]=new Option(ar[i],ar[i]); 
